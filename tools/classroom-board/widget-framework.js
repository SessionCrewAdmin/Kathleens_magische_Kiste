...
function ensureStyles(){
 if(document.getElementById('kathleenWidgetStylesV235'))return;
 const s=document.createElement('style');s.id='kathleenWidgetStylesV235';s.textContent=`
.kwHost,.kwHost *{box-sizing:border-box}.kwHost{width:100%;height:100%;font-family:Inter,ui-rounded,"SF Pro Rounded",Arial,sans-serif;color:#4f3d51;container-type:size}
...
.kwWidgetTop{display:flex;align-items:center;gap:7px;padding:9px 11px;border-bottom:1px solid #eee4ef;background:rgba(255,255,255,.64);min-height:38px}.kwWidgetTop b{font-size:clamp(12px,3.5cqw,24px);flex:1}.kwWidgetIcon{font-size:clamp(18px,4cqw,30px)}.kwWidgetState{font-size:clamp(7px,1.9cqw,11px);font-weight:950;letter-spacing:.07em;text-transform:uppercase;color:#8e7694}
...
.kwBtn,.kwActions button,.kwMiniBtn{border:1px solid #dfd0e1;background:rgba(255,255,255,.88);color:#654f69;border-radius:10px;min-height:32px;padding:6px 9px;font:inherit;font-size:clamp(10px,2.5cqw,16px);font-weight:900;cursor:pointer}.kwBtn:hover,.kwActions button:hover,.kwMiniBtn:hover{background:#fff}.kwBtn.primary,.kwActions button.primary{border:0;color:#fff;background:linear-gradient(135deg,#d58fb6,#ad91df)}.kwBtn.danger{color:#b64960;background:#fff7f8}
.kwField{display:grid;gap:4px;font-size:8px;font-weight:900;color:#806c84}.kwField input,.kwField select,.kwField textarea,.kwInput{width:100%;border:1px solid #ded0e0;border-radius:9px;background:#fff;padding:7px;color:#58475b;font:inherit;font-size:clamp(10px,2.4cqw,16px)}.kwField textarea{min-height:70px;resize:vertical}.kwGrid2{display:grid;grid-template-columns:1fr 1fr;gap:7px}.kwCheck{display:flex;align-items:center;gap:6px;font-size:8px;font-weight:900}.kwEmpty{height:100%;display:grid;place-items:center;text-align:center;color:#9a869e;font-size:clamp(11px,2.8cqw,18px);padding:14px}
...
.kwPollQuestion{font-size:18px;font-weight:950;text-align:center;margin:4px 0 12px;line-height:1.15;letter-spacing:-.02em}
.kwPollSetup{display:grid;gap:7px}.kwPollSetup input,.kwPollSetup select,.kwPollSetup textarea{width:100%;border:1px solid #ded0e0;border-radius:9px;padding:7px;background:#fff;color:#58475b;font:inherit;font-size:9px}.kwPollSetup textarea{min-height:72px;resize:vertical}
.kwPollSetupRow{display:grid;grid-template-columns:1fr 1fr;gap:6px}.kwPollTypeHint{font-size:7px;color:#8f7c93;line-height:1.35}
.kwPollChartSwitch{display:flex;gap:4px;flex-wrap:wrap;margin:8px 0}.kwPollChartSwitch button{border:1px solid #e1d5e3;background:#fff;border-radius:8px;padding:5px 7px;font-size:7px;font-weight:900;color:#715d75}.kwPollChartSwitch button.active{background:#eee5f3;border-color:#c8b1d0}
.kwPollOptions{display:grid;gap:6px}.kwPollOption{display:grid;grid-template-columns:28px minmax(70px,1.15fr) 2fr 34px;align-items:center;gap:6px;padding:5px;border-radius:10px;background:rgba(255,255,255,.56);border:1px solid #eee4ef}.kwPollKey{width:28px;height:28px;border-radius:9px;display:grid;place-items:center;background:linear-gradient(135deg,#d98ab3,#a98ddd);color:#fff;font-size:8px;font-weight:950}.kwPollLabel{font-size:8px;font-weight:850;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.kwPollBar{height:11px;background:#eee5ef;border-radius:999px;overflow:hidden}.kwPollBar i{display:block;height:100%;background:linear-gradient(90deg,#da91b9,#a78bdc);border-radius:999px;transition:width .35s}.kwPollCount{text-align:right;font-size:8px;font-weight:950}
.kwPollCards{display:grid;grid-template-columns:1fr 1fr;gap:6px}.kwPollCard{border:1px solid #e7d9e8;border-radius:12px;background:#fff;padding:8px;min-height:72px}.kwPollCard b{display:block;font-size:8px}.kwPollCard strong{font-size:22px;line-height:1.1}.kwPollCard small{font-size:7px;color:#8e7a92}
.kwPollPieWrap{display:grid;grid-template-columns:120px 1fr;gap:10px;align-items:center}.kwPollPie{width:112px;height:112px;border-radius:50%;position:relative;margin:auto}.kwPollPie.donut:after{content:"";position:absolute;inset:27%;border-radius:50%;background:#fffafd;border:1px solid #ede2ee}.kwPollPieCenter{position:absolute;inset:0;display:grid;place-items:center;z-index:2;font-size:15px;font-weight:950}.kwPollPie:not(.donut) .kwPollPieCenter{display:none}.kwPollLegend{display:grid;gap:4px}.kwPollLegendRow{display:grid;grid-template-columns:9px 1fr auto;gap:5px;align-items:center;font-size:7px}.kwPollLegendDot{width:9px;height:9px;border-radius:3px}
.kwPollWordcloud{min-height:115px;display:flex;flex-wrap:wrap;gap:7px 10px;align-content:center;justify-content:center;padding:8px}.kwPollWordcloud span{font-weight:950;color:#684f6d}.kwPollAnswers{display:grid;gap:5px}.kwPollAnswer{border:1px solid #e6d9e8;background:#fff;border-radius:9px;padding:7px;font-size:8px;line-height:1.35}.kwPollRanking{display:grid;gap:5px}.kwPollRank{display:grid;grid-template-columns:28px 1fr auto;gap:6px;align-items:center;border:1px solid #e6d9e8;background:#fff;border-radius:9px;padding:6px}.kwPollRank i{width:25px;height:25px;border-radius:8px;background:#efe6f3;display:grid;place-items:center;font-style:normal;font-size:8px;font-weight:950}.kwPollRank b{font-size:8px}.kwPollRank small{font-size:7px;color:#89758d}
.kwPollStudentAnswer{display:grid;gap:6px}.kwPollStudentOption{position:relative;border:1px solid #dfd2e2;background:#fff;border-radius:11px;padding:9px 10px 9px 39px;text-align:left;color:#58475b;font-size:9px;font-weight:850;min-height:38px}.kwPollStudentOption:before{content:attr(data-letter);position:absolute;left:7px;top:50%;transform:translateY(-50%);width:24px;height:24px;border-radius:8px;display:grid;place-items:center;background:linear-gradient(135deg,#d98ab3,#a98ddd);color:#fff;font-size:7px;font-weight:950}.kwPollStudentOption.selected{background:#f1e8f4;border-color:#bea3c7;box-shadow:0 0 0 2px #d7bfdc55}.kwPollStudentOption.selected:after{content:"✓";position:absolute;right:9px;top:50%;transform:translateY(-50%);font-weight:950}
...
.kwSeatingHead{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px}.kwSeatingHead strong{font-size:clamp(22px,4cqw,34px)}.kwSeatingHead small{display:block;font-size:clamp(13px,2.2cqw,18px);color:#8d788f;margin-top:3px}.kwRoomCode{font-size:clamp(14px,2.3cqw,19px);font-weight:950;letter-spacing:.12em;padding:7px 10px;border-radius:10px;background:#f3edff;color:#6746aa}.kwSeatingRoom{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;padding:10px;border:1px solid #eadde9;border-radius:16px;background:linear-gradient(180deg,#f8e3c2,#efc994);min-height:300px}.kwDeskMini{display:grid;grid-template-columns:1fr 1fr;gap:5px;padding:7px;border:1px solid #d4aa76;border-radius:9px;background:#e5bc83;min-height:64px}.kwDeskMini.single{grid-template-columns:1fr}.kwDeskMini.four{grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr}.kwDeskMini.row4{grid-template-columns:repeat(4,1fr)}.kwSeatMini{border:2px solid #ddd0e2;border-radius:8px;background:#fff;color:#503d51;font-size:clamp(12px,2cqw,17px);font-weight:900;min-height:42px;padding:5px}.kwSeatMini.online{border-color:#61b88d;background:#f1fbf6}.kwSeatMini.offline{border-color:#d96b79;background:#fff3f5}.kwSeatMini.waiting{border-color:#c7bcc9;background:#faf8fb}.kwSeatMini.empty{opacity:.6}
`;
...
function mount(container,item,ctx={}){
 ...
 if(item.widgetType==='poll'&&ctx.mode==='student'){
   const emit=value=>container.dispatchEvent(new CustomEvent('kathleen:widget-action',{bubbles:true,detail:{type:'poll',action:'poll-submit',value}}));
   const multi=new Set();
   container.querySelectorAll('[data-poll-choice]').forEach(btn=>btn.addEventListener('click',ev=>{ev.stopPropagation();const type=btn.dataset.pollType||'choice',v=+btn.dataset.pollChoice;if(type==='multi'){if(multi.has(v))multi.delete(v);else multi.add(v);btn.classList.toggle('selected',multi.has(v))}else{container.querySelectorAll('[data-poll-choice]').forEach(x=>x.classList.remove('selected'));btn.classList.add('selected');emit({answerValue:String(v),answerText:null})}}));
   container.querySelectorAll('[data-poll-star]').forEach(btn=>btn.addEventListener('click',ev=>{ev.stopPropagation();container.querySelectorAll('[data-poll-star]').forEach(x=>x.classList.remove('selected'));btn.classList.add('selected');emit({answerValue:String(btn.dataset.pollStar),answerText:null})}));
   const submit=container.querySelector('[data-poll-submit]');if(submit)submit.addEventListener('click',ev=>{ev.stopPropagation();const type=submit.dataset.pollSubmit;if(type==='multi'){if(!multi.size)return;emit({answerValue:JSON.stringify([...multi].sort((a,b)=>a-b)),answerText:null})}else if(type==='ranking'){const order=[...container.querySelectorAll('[data-poll-rank]')].map(x=>+x.dataset.pollRank);emit({answerValue:JSON.stringify(order),answerText:null})}else{const inp=container.querySelector('[data-poll-text]');const value=inp?.value?.trim()||'';if(value)emit({answerValue:null,answerText:value})}});
   container.querySelectorAll('[data-poll-rank-up],[data-poll-rank-down]').forEach(btn=>btn.addEventListener('click',ev=>{ev.stopPropagation();const row=btn.closest('[data-poll-rank]'),list=row?.parentElement;if(!row||!list)return;if(btn.hasAttribute('data-poll-rank-up')&&row.previousElementSibling)list.insertBefore(row,row.previousElementSibling);if(btn.hasAttribute('data-poll-rank-down')&&row.nextElementSibling)list.insertBefore(row.nextElementSibling,row)}));
 }
...
function renderRandom(data,{mode,esc}){
 const teacher=mode==='teacher',names=arr(data.names),drawn=arr(data.drawn),result=arr(data.result),classes=arr(data.classOptions),left=data.excludeDrawn===false?names.length:names.filter(n=>!drawn.includes(n)).length;
 const classSelect=teacher?'<label class="kwField">Klasse<select data-kw-field="sourceClassId">'+(classes.length?classes.map(c=>'<option value="'+esc(c.id)+'"'+(String(data.sourceClassId||'')===String(c.id)?' selected':'')+'>'+esc(c.name)+'</option>').join(''):'<option value="">Keine Klassen geladen</option>')+'</select></label>':'';
 return '<div class="kwWidget"><div class="kwWidgetTop"><span class="kwWidgetIcon">🎯</span><b>Zufall</b><span class="kwWidgetState">'+esc(data.sourceName||names.length+' Namen')+'</span></div><div class="kwBody">'+(teacher?'<div class="kwRandomSource">'+classSelect+'</div>':'')+'<div class="kwRandomHero'+(data.spinning?' spinning':'')+'"><strong>'+(result.length?result.map(esc).join('<br>'):'Bereit?')+'</strong></div><div class="kwRandomMeta">'+names.length+' Schüler · '+(data.excludeDrawn===false?'Wiederholungen erlaubt':left+' im Pool')+'</div>'+(drawn.length?'<div class="kwHistory">'+drawn.slice(-12).map(n=>'<span class="kwChip">'+esc(n)+'</span>').join('')+'</div>':'')+
 ...
 (teacher?'<div class="kwActions"><button class="primary" data-kw-action="random-draw">🎲 Ziehen</button><button data-kw-action="random-reset">↺ Pool</button><button data-kw-action="source-refresh">↻ Klassen neu laden</button></div>':'')+'</div>';
}
...
function renderSeating(data,{mode,esc}){
 const teacher=mode==='teacher',desks=Array.isArray(data.desks)?data.desks:[],participants=Array.isArray(data.participants)?data.participants:[],byName=new Map(participants.map(p=>[String(p.name||''),p])),online=participants.filter(p=>p.online).length,roster=Number(data.rosterCount)||0;
 const deskHtml=desks.slice(0,24).map(d=>{const names=Array.isArray(d.names)?d.names:[],seats=names.map(n=>{const p=byName.get(String(n||'')),state=!n?'empty':!p?'waiting':p.online?'online':'offline',label=n?String(n).trim().split(/\s+/)[0]:'Frei';return '<button type="button" class="kwSeatMini '+state+'" title="'+esc(n||'')+'" data-kw-action="seating-student" data-kw-value="'+esc(n||'')+'" '+(!n?'disabled':'')+'><span>'+esc(label)+'</span></button>'}).join('');return '<div class="kwDeskMini '+esc(d.type||'two')+'">'+seats+'</div>'}).join('');
 return '<div class="kwWidget kwSeating"><div class="kwWidgetTop"><span class="kwWidgetIcon">🪑</span><b>Sitzplan</b><span class="kwWidgetState">'+(data.classroomActive?'LIVE':'BEREIT')+'</span></div><div class="kwBody"><div class="kwSeatingHead"><div><strong>'+esc(data.className||'Klasse')+'</strong><small>'+(data.classroomActive?online+' / '+(roster||participants.length)+' verbunden':'Sitzplanübersicht')+'</small></div>'+(data.roomCode?'<span class="kwRoomCode">'+esc(data.roomCode)+'</span>':'')+'</div><div class="kwSeatingRoom">'+(deskHtml||'<div class="kwEmpty">Noch kein Sitzplan gespeichert.</div>')+'</div></div>'+(teacher?'<div class="kwActions"><button class="primary" data-kw-action="seating-open">🪑 Sitzplan öffnen</button><button data-kw-action="seating-refresh">↻ Aktualisieren</button></div>':'')+'</div>';
}
...
register({type:'timer',icon:'⌛',title:'Timer',defaultSize:{w:560,h:390},defaultData:{title:'Timer',seconds:300,duration:300,remaining:300,running:false,endsAt:null,alarm:true},render:renderTimer});
register({type:'traffic',icon:'🚦',title:'Ampel',defaultSize:{w:610,h:460},defaultData:{title:'Ampel',active:'green',variant:'classic',labels:{red:'Nicht reden',yellow:'Flüsterstimme',green:'Innenstimme'},showLabels:true},render:renderTraffic});
register({type:'random',icon:'🎯',title:'Zufallsgenerator',defaultSize:{w:590,h:500},defaultData:{names:[],sourceName:'',sourceClassId:'',classOptions:[],excludeDrawn:true,count:1,drawn:[],result:[]},render:renderRandom});
...
register({type:'seating',icon:'🪑',title:'Sitzplan',defaultSize:{w:760,h:560},defaultData:{className:'',classId:'',desks:[],participants:[],rosterCount:0,classroomActive:false,roomCode:''},render:renderSeating});
