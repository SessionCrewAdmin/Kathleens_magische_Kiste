...
function renderSeating(data,{mode,esc}){
 ...
 const deskHtml=desks.slice(0,18).map(d=>{const names=Array.isArray(d.names)?d.names:[],seats=names.map(n=>{const p=byName.get(String(n||'')),state=!n?'empty':!p?'waiting':p.online?'online':'offline';return '<button type="button" class="kwSeatMini '+state+'" data-kw-action="seating-student" data-kw-value="'+esc(n||'')+'" '+(!n?'disabled':'')+'><span>'+esc(n||'Frei')+'</span></button>'}).join('');return '<div class="kwDeskMini '+esc(d.type||'two')+'">'+seats+'</div>'}).join('');
