...
async function loadChapter(ch){
 ...
 data=await getJson(base+ch.data+'?v=20260925-history-full');tab='topics';renderGrades();renderChapters();render();
...
async function init(){
 try{
  catalog=await getJson(base+'catalog.json?v=20260925-history-full');
...
function renderChapters(){
 ...
 $('#chapters').innerHTML=items.map(c=>'<button class="chapter '+(c.status==='ready'?'ready':'')+' '+(c.id===activeId?'active':'')+'" data-id="'+esc(c.id)+'" '+(c.status!=='ready'||!c.data?'disabled':'')+'><b>'+esc(c.title)+'</b><span>'+(c.status==='ready'?'Full Import · '+esc(c.book_pages||''):'Import vorbereitet')+'</span></button>').join('');
...
function setStats(){
 ...
 const a=data.full_import_audit||{};$('#importState').textContent=data.import_status==='full_import'?'✓ Full Import · '+(a.files_audited||0)+'/'+(a.files_expected||0)+' Dateien':'Import läuft'
}
...
function render(){
 ...
 if(tab==='questions')body=allQuestions().filter(match).map(q=>'<div class="question"><small>'+esc(q.difficulty||'standard')+' · '+esc(q.type||'question')+'</small><b>'+esc(q.prompt)+'</b><p>Antwort: '+esc(Array.isArray(q.answer)?q.answer.join(' → '):q.answer)+'</p></div>').join('');
 ...
 if(tab==='audit'){const a=data.full_import_audit||{};body='<div class="card"><h3>Full-Import-Audit</h3><p>'+esc(a.note||'')+'</p><div class="tags">'+(a.coverage||[]).map(x=>'<span class="tag">✓ '+esc(x)+'</span>').join('')+'</div></div><div class="auditrow"><b>Dateien</b><span>'+esc(a.files_audited||0)+' von '+esc(a.files_expected||0)+' geprüft</span></div><div class="auditrow"><b>Originalmaterial</b><span>PDFs, Word-Dateien und Lehrwerksbilder bleiben außerhalb des öffentlichen Repositories.</span></div>'}
