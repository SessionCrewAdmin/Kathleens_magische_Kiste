...
function ensureSwitcher(){
 ...
 const s=document.createElement('style');s.textContent='.kathleen-lang-switch{position:fixed;right:max(12px,env(safe-area-inset-right));bottom:max(12px,env(safe-area-inset-bottom));z-index:24000;display:flex;gap:3px;padding:4px;border:1px solid #e8dbe7;border-radius:999px;background:rgba(255,255,255,.94);box-shadow:0 10px 30px rgba(80,57,82,.16);backdrop-filter:blur(10px)}.kathleen-lang-switch button{border:0;border-radius:999px;background:transparent;color:#806b82;padding:7px 9px;font:900 9px/1 Inter,Arial,sans-serif;min-height:32px}.kathleen-lang-switch button.active{background:#725778;color:#fff}.kathleen-lang-switch .auto{padding-inline:8px}@media(max-width:820px){.kathleen-lang-switch{top:calc(8px + env(safe-area-inset-top));right:max(9px,env(safe-area-inset-right));bottom:auto;z-index:27000;padding:3px}.kathleen-lang-switch button{min-height:32px;padding:6px 7px}.mobile-teacher-v36 .kathleen-lang-switch{top:calc(10px + env(safe-area-inset-top));right:62px}.mobile-teacher-v36 .kathleen-lang-switch .auto{display:none}}';document.head.appendChild(s);
...
function updateSwitcher(){const d=document.getElementById('kathleenLangSwitch');if(!d)return;const lang=resolve(),ov=localStorage.getItem(OVERRIDE_KEY);d.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.lang===(ov?lang:'auto')));d.querySelector('.auto').title=lang==='en'?'Automatic · English':'Automatisch · Deutsch'}
function apply(){
